package fr.pts2.usermanagement.fixedconstraints;
...
import fr.pts2.usermanagement.fixedconstraints.add.AddFixedConstraints;
...
public class FixedConstraints implements Initializable {
	...
	public void deleteFixedConstraint() {
		...
		if (tvc != null) {
			FixedConstraintHandler.deleteFixedConstraint(UserManagement.user, Days.fromString(tvc.getDay()).ordinal()+1, Intervals.fromString(tvc.getInterval()).ordinal()+1);
	...
	private void setupTableView() {
		for (Constraint fc : FixedConstraintHandler.getFixedConstraints(UserManagement.user)) {
			Utils.log(""+UserManagement.user);
